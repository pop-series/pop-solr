...
public class ReadSimulation extends Simulation {
    ...
    final String fixedQueryParams = "q=*:*&fl=id,name,author,field(popularity)&sort=popularity asc,id desc";
    ...
    final ScenarioBuilder scn = scenario("Sort by External File Field")
            .feed(queryFeeder)
            .exec(http("eff:popularity")
                    .get("?#{query}&" + fixedQueryParams)
                    .check(status().is(200))
